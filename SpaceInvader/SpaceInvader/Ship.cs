...
namespace SpaceInvader
{
    ...
    class Ship
    {
        ...
        public Ship(KeyboardManager km, SpriteBatch spriteBatch, ContentManager content, GraphicsDevice graphicsDevice, List<Alien> aliens)
        {
            ...
            shipTexture = content.Load<Texture2D>("tankRed");
            bulletsound = content.Load<SoundEffect>("laser");
            playerpos = new Vector2(0, shipTexture.Height / 2);
            Point p = new Point(shipTexture.Width, shipTexture.Height);
            shiphitbox = new Rectangle(playerpos.ToPoint(), p);
        ...
        {
            if (shiphitbox.Intersects(hitbox))
                return true;
            else return false;
        }
